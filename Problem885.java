...
public class Problem885 {
    public int[][] spiralMatrixIII(int rows, int cols, int rStart, int cStart) {
        ...
        while (totalSteps < (rows * cols)-1) {
            ...
            int j = 0;
            while (j < i) {
                currCols = currCols + neg;
                if (currCols >= 0 && currCols < cols && currRows >= 0 && currRows < rows) {

                    list.add(new int[] { currRows, currCols });
                    totalSteps++;
                }
    ...
    public static void main(String[] args) {
        ...
        int[][] arr = problem885.spiralMatrixIII(rows, cols, rStart, cStart);
        for (int[] a : arr) {
            for (int b : a) {
                System.out.print(b + " ");
            }
